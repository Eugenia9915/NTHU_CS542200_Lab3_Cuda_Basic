...
void sobel (unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels) {
    ...
    // parallel job by blockIdx, blockDim, threadIdx 
        ...
        for(y= blockIdx.x * blockDim.y + threadIdx.y ; y < height; y += blockDim.y * gridDim.x ){
...
int main(int argc, char** argv) {
    ...
    unsigned char* host_r = (unsigned char*) malloc(height * width * channels * sizeof(unsigned char));
    
    unsigned char *device_s = NULL, *device_d = NULL;
    /* Hint 1 */
    // cudaMalloc(...) for device src and device dst
    cudaMalloc((void**)&device_s, height * width * channels * sizeof(unsigned char));
    cudaMalloc((void**)&device_d, height * width * channels * sizeof(unsigned char));
    ...
    //sobel<<<height/20+1,20>>>(device_s, device_d, height, width, channels);

    dim3 num_threads(128,4);
    dim3 num_blocks = 2048;
    sobel<<<num_blocks,num_threads>>>(device_s, device_d, height, width, channels);

    /* Hint 4 */
    // cudaMemcpy(...) copy result image to host
    cudaMemcpy(host_r, device_d, height * width * channels * sizeof(unsigned char),cudaMemcpyDeviceToHost);
    write_png(argv[2], host_r, height, width, channels);
    cudaFree(device_s); cudaFree(device_d); 
    return 0;
}
